...
@pytest.mark.usefixtures('restart_api')
def test_unhappy_invalid_sku_returns_400_and_error_message():
...
@pytest.mark.usefixtures('restart_api')
def test_unhappy_out_of_stock_returns_400_and_error_message(add_stock):
    sku, orderid = random_sku(), random_orderid()
    batch = random_batchref(1)
    add_stock([(batch, sku, 10, None)])
